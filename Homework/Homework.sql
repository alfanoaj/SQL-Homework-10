...
alter table actor
	add column `middle_name` varchar(20) not null after `first_name`;
...
select customer.customer_id, customer.first_name, customer.last_name, sum(payment.amount)  as 'Amount Paid'
from customer 
join payment
...
select title, count(rental.inventory_id) as 'Times Rented' 
from film
	join inventory on
		film.film_id = inventory.film_id
	join rental on
...
select staff.store_id, sum(payment.amount) as 'Store Sales ($)'
from staff
inner join payment 
on staff.staff_id=payment.staff_id
group by store_id;
